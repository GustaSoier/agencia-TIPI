    ...
    if(isset($_POST['TituloGaleria'])){
        ...
        if(!empty($_FILES['imgGaleria']['name'])){
            ...
            if(move_uploaded_file($arquivo['tmp_name'], '../img/' . $arquivo['name'])){
                $imgServico = 'galeria/' . $arquivo['name'];
            }
            else{
...
<div class="container-inserir">
    <form action="index.php?p=galeria&a=atualizar$id=<?php echo $galeria -> idGaleria ?>" method="POST" enctype="multipart/form-data">


            <label for="foto">Escolher Foto:</label>
            <div class="foto-preview">
                <img src="<?php echo '../img/' . $galeria -> imgGaleria ?>" alt="" id="imagemExibida">
            </div>
            ...
            <input type="text" id="tituloServico" name="tituloServico" value="<?php echo $galeria -> tituloGaleria ?>" required>
        
            <h3>Status Galeria:</h3>
            <input type="checkbox" id="status" name="statusServico" value="<?php echo $galeria -> statusGaleria ?>" required>
             <label for="status"></label>
        

            <label for="link">Link do Galeria:</label>
            <input type="text" id="link" name="linkServico" value="<?php echo $galeria -> linkGaleria ?>" required>
